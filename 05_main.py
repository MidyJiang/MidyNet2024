...
class Trainer:
    ...
    def _run_epoch(self, epoch):
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}] \t[GPU{self.gpu_id}] \
        Epoch {epoch} | Batchsize: {b_sz} | Steps: {len(self.train_data)} | loss: {loss}")
    ...
    def _eval_epoch(self, epoch):
        ...
        # print(110, len(loss), loss)
        loss = sum(loss) / len(loss)
        calc_metric(mat, loss, epoch, self.usewb,mode='dev')
    ...
    def train(self, max_epochs: int):
        for epoch in range(max_epochs):
            ...
            if self.gpu_id == 0 and epoch % self.check_every == 0:
                ...
                print(f"loss={loss:.4f},best_loss={self.best_loss:.4f}")
...
def calc_metric(mat, loss, epoch=0, usewb=False, mode='dev'):  # positive = down, negative = up
    ...
    mcc = (tp * tn - fp * fn) / torch.sqrt(((tp + fp) * (tp + fn)
                                            * (tn + fp) * (tn + fn)))
    print(f'[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))}]\t{mode}_loss: {loss}')
    print(
        f'[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))}]\t(Confusion Matrix)\tTP={tp}, FN={fn}, FP={fp}, TN={tn}')
    print(
        f'[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))}]\t(Eval Matrix)\tacc={acc}, rec={rec}, pre={pre}, f1={f1}, mcc={mcc}')
...
def test(flags, save_name):
    ...
    model = MidyNet(flags).to(device=flags.device)
...
def main(rank: int, world_size: int, flags):
    ...
    dev_loader = DataLoader(dataset=dev_data, batch_size=flags.batch_size,
                            num_workers=flags.num_workers, shuffle=False,
                            pin_memory=True, drop_last=True)
    print(215,len(train_loader),len(dev_loader))
...
def init_wandb(flags):
    import wandb
    from wandb_setting import ini,wandb_config
    import random
    import string

    exp_name = ''.join(random.choices(string.ascii_uppercase + string.digits, k=3))  #"Midy_HAN_CMIN"
    print(f"experiment random 8name: {exp_name}")
    with open('exp_name_temp.txt', 'w') as f:
        f.write(exp_name)  # write to file
...
if __name__ == "__main__":
    ...
    flags = config.args
    print(235, flags.device)
    world_size = torch.cuda.device_count()

    # mp.spawn(main, args=(world_size, flags), nprocs=world_size) # parallel
    main(0, world_size, flags)# replace if non-parallel

    print(242, time.ctime(), 'finished and success。done')
